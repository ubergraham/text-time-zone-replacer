...
var ENTimeUnitWithinFormatParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match2) {
    ...
    const timeUnits = parseDuration(match2[1]);
    if (!timeUnits) {
      return null;
    }
...
var ENMonthNameLittleEndianParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match2) {
    ...
    const day = parseOrdinalNumberPattern2(match2[DATE_GROUP]);
    if (day > 31) {
    ...
    if (match2[YEAR_GROUP]) {
      const yearNumber = parseYear2(match2[YEAR_GROUP]);
    ...
    if (match2[DATE_TO_GROUP]) {
      const endDate = parseOrdinalNumberPattern2(match2[DATE_TO_GROUP]);
...
var ENMonthNameMiddleEndianParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match2) {
    ...
    const day = parseOrdinalNumberPattern2(match2[DATE_GROUP2]);
    if (day > 31) {
      return null;
    }
    ...
    if (match2[YEAR_GROUP2]) {
      const year = parseYear2(match2[YEAR_GROUP2]);
    ...
    const endDate = parseOrdinalNumberPattern2(match2[DATE_TO_GROUP2]);
    const result = context.createParsingResult(match2.index, match2[0]);
...
var ENMonthNameParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match2) {
    ...
    if (match2[YEAR_GROUP3]) {
      const year = parseYear2(match2[YEAR_GROUP3]);
...
var AbstractTimeExpressionParser = class {
  ...
  extractPrimaryTimeComponents(context, match2, strict2 = false) {
    ...
    if (hour > 100) {
      if (match2[HOUR_GROUP].length == 4 && match2[MINUTE_GROUP] == null && !match2[AM_PM_HOUR_GROUP]) {
        return null;
      }
...
var ENTimeExpressionParser = class extends AbstractTimeExpressionParser {
  ...
  extractFollowingTimeComponents(context, match2, result) {
    const followingComponents = super.extractFollowingTimeComponents(context, match2, result);
...
var ENTimeUnitAgoFormatParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match2) {
    const duration = parseDuration(match2[1]);
    if (!duration) {
      return null;
    }
...
var AbstractMergeDateRangeRefiner = class extends MergingRefiner {
  ...
  mergeResults(textBetween, fromResult, toResult) {
    ...
    if (fromResult.start.date() > toResult.start.date()) {
      ...
      let toDate3 = toResult.start.date();
      if (toResult.start.isOnlyWeekdayComponent() && addDuration(toDate3, { day: 7 }) > fromDate) {
        toDate3 = addDuration(toDate3, { day: 7 });
        toResult.start.imply("day", toDate3.getDate());
        toResult.start.imply("month", toDate3.getMonth() + 1);
        toResult.start.imply("year", toDate3.getFullYear());
      } else if (fromResult.start.isOnlyWeekdayComponent() && addDuration(fromDate, { day: -7 }) < toDate3) {
      ...
      } else if (toResult.start.isDateWithUnknownYear() && addDuration(toDate3, { year: 1 }) > fromDate) {
        toDate3 = addDuration(toDate3, { year: 1 });
        toResult.start.imply("year", toDate3.getFullYear());
      } else if (fromResult.start.isDateWithUnknownYear() && addDuration(fromDate, { year: -1 }) < toDate3) {
...
var ENTimeUnitCasualRelativeFormatParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match2) {
    ...
    let duration = parseDuration(match2[2]);
    if (!duration) {
      return null;
    }
    switch (prefix) {
...
var ENExtractYearSuffixRefiner = class {
  refine(context, results) {
    results.forEach(function(result) {
      ...
      const year = parseYear2(match2[YEAR_GROUP6]);
      if (result.end != null) {
...
// timeParser.ts
var RANGE_SEPARATOR_RE = /\s*(?:–|—|\s+to\s+|\s+through\s+|\s+until\s+)\s*|(?<=\w)\s*-\s*(?=\d)/i;
...
function parseTimeInput(input) {
  ...
  const chronoResults = parse2(trimmed);
  ...
  if (separatorMatch && separatorMatch.index !== void 0) {
    ...
    const endStr = trimmed.slice(separatorMatch.index + separatorMatch[0].length).trim();
    if (startStr && endStr) {
      let dateReference = now2;
      if (includesDate) {
        const dateContext = parse2(trimmed);
...
function parseSingleTime(input, includesDate, includesYear) {
  ...
  switch (lowered) {
    ...
    case "noon":
      return {
        date: set(now2, { hours: 12, minutes: 0, seconds: 0, milliseconds: 0 }),
        includesDate: false,
        includesYear: false,
        isRange: false
      };
    case "midnight":
      return {
        date: set(now2, { hours: 0, minutes: 0, seconds: 0, milliseconds: 0 }),
        includesDate: false,
        includesYear: false,
        isRange: false
      };
  }
  const chronoParsed = parse2(input);
  ...
  return {
    date: parseFallback(input, now2),
    includesDate: false,
    includesYear: false,
    isRange: false
  };
}
function parseSingleTimeWithReference(input, reference) {
  const lowered = input.toLowerCase().trim();
  switch (lowered) {
    case "noon":
      return {
  ...
  const chronoParsed = parse2(input, reference);
  if (chronoParsed.length > 0) {
    return {
      date: chronoParsed[0].start.date(),
...
function parseFallback(input, reference) {
  const lowered = input.toLowerCase().trim();
  if (lowered.includes("am") || lowered.includes("pm")) {
    let parsed2 = parse(input, "h:mm a", reference);
    if (isValid(parsed2)) return parsed2;
    parsed2 = parse(input, "h:mma", reference);
    if (isValid(parsed2)) return parsed2;
    parsed2 = parse(input, "ha", reference);
    if (isValid(parsed2)) return parsed2;
    parsed2 = parse(input, "h a", reference);
    if (isValid(parsed2)) return parsed2;
  }
  if (input.includes(":")) {
    const parsed2 = parse(input, "HH:mm", reference);
    if (isValid(parsed2)) return parsed2;
  }
...
try {
  ...
  process.stderr.write(`time-converter: ${message2}
`);
  process.exit(1);
}
